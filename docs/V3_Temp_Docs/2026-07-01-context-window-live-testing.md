...
| 4 | Dead `apperr.ContextWindow`; friendly toast unreachable | T64 | **FIXED** | Forced a genuine overflow (LM Studio `google/gemma-3-1b` loaded with `-c 512`, ~1,468-token input). GoText backend log: `[ActionService.runStep] LLM call failed family=summarize error=The text exceeds the model's context window.` — the exact message wired in `notifications/slice.ts`'s `CodeContextWindow` case ("Input too long" / "The text exceeds the model's context window — shorten it or raise the context size."), not the old generic Upstream/"Something went wrong" path. |
