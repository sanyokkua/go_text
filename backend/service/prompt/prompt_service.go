...
func (p *promptServiceStruct) GetPrompt(promptId string) (model.Prompt, error) {
	startTime := time.Now()
	p.logger.Info(fmt.Sprintf("[GetPrompt] Fetching prompt with ID: %s", promptId))

	prompt, err := p.GetUserPromptById(promptId)
	if err != nil {
		p.logger.Error(fmt.Sprintf("[GetPrompt] Failed to get prompt with ID '%s': %v", promptId, err))
		return model.Prompt{}, fmt.Errorf("failed to retrieve prompt with ID '%s': %w", promptId, err)
	}

	duration := time.Since(startTime)
	p.logger.Info(fmt.Sprintf("[GetPrompt] Successfully retrieved prompt '%s' in %v", promptId, duration))
...
func (p *promptServiceStruct) GetSystemPrompt(category string) (string, error) {
	startTime := time.Now()
	p.logger.Info(fmt.Sprintf("[GetSystemPrompt] Fetching system prompt for category: %s", category))

	systemPrompt, err := p.GetSystemPromptByCategory(category)
	if err != nil {
		p.logger.Error(fmt.Sprintf("[GetSystemPrompt] Failed to get system prompt for category '%s': %v", category, err))
		return "", fmt.Errorf("failed to retrieve system prompt for category '%s': %w", category, err)
	}

	duration := time.Since(startTime)
	p.logger.Info(fmt.Sprintf("[GetSystemPrompt] Successfully retrieved system prompt for category '%s' in %v", category, duration))
...
func (p *promptServiceStruct) BuildPrompt(template, category string, action *action.ActionRequest, useMarkdown bool) (string, error) {
	startTime := time.Now()
	p.logger.Info(fmt.Sprintf("[BuildPrompt] Building prompt for category: %s, ActionID: %s", category, action.ID))

	if action == nil {
		errorMsg := "action is nil"
		p.logger.Error(fmt.Sprintf("[BuildPrompt] %s", errorMsg))
	...
	if !isValidAction {
		p.logger.Error(fmt.Sprintf("[BuildPrompt] Action validation failed: %v", err))
	...
	for token, val := range replacements {
		...
		if err != nil {
			p.logger.Error(fmt.Sprintf("[BuildPrompt] Failed to replace template parameter '%s': %v", token, err))
			return "", fmt.Errorf("template parameter replacement failed for '%s': %w", token, err)
		}
		p.logger.Trace(fmt.Sprintf("[BuildPrompt] Replaced parameter '%s' in template. Before: %.50s..., After: %.50s...", token, originalTemplate, template))
	}

	duration := time.Since(startTime)
	p.logger.Info(fmt.Sprintf("[BuildPrompt] Successfully built prompt in %v, Final length: %d characters", duration, len(template)))
