...
func (l llmHttpService) buildRequestURL(baseUrl, endpoint string) (string, error) {
	if l.stringUtils.IsBlankString(baseUrl) {
		return "", fmt.Errorf("base URL cannot be empty or whitespace")
	}
	if l.stringUtils.IsBlankString(endpoint) {
...
func NewLlmHttpApiService(logger backend.LoggingApi, client *resty.Client, stringUtils backend.StringUtilsApi) backend.LlmHttpApi {
