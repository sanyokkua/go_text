...
func TestCompletionRequest_Success(t *testing.T) {
	t.Run("Successful completion response", func(t *testing.T) {
		...
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			// Verify request body
			var req llm2.ChatCompletionRequest
			...
			response := llm2.ChatCompletionResponse{
				ID:    "resp-123",
				Model: "gpt-3.5-turbo",
				Choices: []llm2.Choice{
					{
						Index:        0,
						Message:      llm2.Message{Role: "assistant", Content: "Hello there!"},
						FinishReason: "stop",
					},
				},
				Usage: llm2.Usage{
...
func TestLoggingBehavior(t *testing.T) {
	t.Run("Info logging should occur for successful requests", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			json.NewEncoder(w).Encode(llm2.LlmModelListResponse{})
		}))
		defer server.Close()
