...
describe('CommandPalette', () => {
    it('calls onSelect when an item is clicked normally', async () => {
        ...
        render(<CommandPalette open={true} onOpenChange={jest.fn()} items={items} onSelect={onSelect} />);
    ...
    it('does NOT call onSelect when disabled and item is clicked', async () => {
        const onSelect = jest.fn();
        render(<CommandPalette open={true} onOpenChange={jest.fn()} items={items} onSelect={onSelect} disabled={true} />);
    ...
    it('shows disabled indicator when disabled', () => {
        render(<CommandPalette open={true} onOpenChange={jest.fn()} items={items} onSelect={jest.fn()} disabled={true} />);
        expect(screen.getByRole('dialog', { name: 'Command palette' })).toHaveAttribute('aria-disabled', 'true');
    });
});
