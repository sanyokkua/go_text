...
function buildStore(aboutOverrides: Record<string, unknown> = {}) {
...
describe('PromptInspector', () => {
    ...
    it('shows loading spinner while inspectorLoading is true', () => {
        const store = buildStore({ selectedItemId: 'a1', selectedItemType: 'action', inspectorLoading: true });
        render(
    ...
    it('shows error message when inspectorError is set', () => {
        const store = buildStore({ selectedItemId: 'a1', selectedItemType: 'action', inspectorError: 'Action not found' });
        render(
            <Provider store={store}>
                <PromptInspector />
            </Provider>,
    ...
    it('renders preview groups when inspectorData is set', () => {
        const store = buildStore({ selectedItemId: 'a1', selectedItemType: 'action', inspectorData: mockPreview });
        render(
            <Provider store={store}>
                <PromptInspector />
            </Provider>,
