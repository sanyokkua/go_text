...
jest.mock('../../../../../logic/adapter', () => ({
    StackHandlerAdapter: {
        ...
        duplicateStack: jest.fn().mockResolvedValue({
            data: { id: 'dup-1', name: 'My Pipeline (copy)', icon: '📝', steps: ['proofread'], defaultFormat: 'PlainText', defaultInLang: '', defaultOutLang: '', createdAt: 0, updatedAt: 0 },
            error: null,
        }),
    },
...
const MOCK_STACK = {
    id: 'stack-1', name: 'My Pipeline', icon: '📝',
...
function makeStore(overrides: StoreOverrides = {}) {
    return configureStore({
        reducer: {
            stacksSaved: stacksSavedReducer, stacksBuilder: stacksBuilderReducer,
        ...
        preloadedState: {
            ...
            ui: {
                ...
                currentView: 'stacks' as const, armedActionId: null, activeActionsTab: null,
                buildMode: false, editingStackId: null,
                    activeSettingsTab: 0,
                theme: { mode: 'auto' as const, effective: 'light' as const },
            },
            run: {
                status: 'idle' as const, runId: null, currentGroupIndex: null, totalGroups: null,
                currentGroupFamily: null, failedIndex: null, partialOutput: null, errorCode: null, errorMessage: null,
            },
...
describe('StacksManageView', () => {
    it('renders "My Stacks" heading', () => {
        render(<Provider store={makeStore()}><StacksManageView /></Provider>);
    ...
    it('clicking back navigates to main view', async () => {
        ...
        render(<Provider store={store}><StacksManageView /></Provider>);
    ...
    it('shows empty state with only the Build tile when no stacks exist', () => {
        render(<Provider store={makeStore({ stacks: [] })}><StacksManageView /></Provider>);
    ...
    it('card Run button is disabled when inferenceRunning', () => {
        render(<Provider store={makeStore({ inferenceRunning: true })}><StacksManageView /></Provider>);
