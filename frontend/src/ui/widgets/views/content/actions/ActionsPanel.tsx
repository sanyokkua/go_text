...
const ActionsPanel: React.FC = () => {
    ...
    const sortedGroups = promptGroups
        ? Object.values(promptGroups.promptGroups).sort((a, b) => a.groupId.localeCompare(b.groupId))
        : [];
    ...
    const handleActionClick = async (actionId: string, promptName: string) => {
        if (isAppBusy || !inputContent) { return; }
        try {
            logger.logInfo(`Action clicked: ${actionId}`);
            dispatch(setCurrentTask(promptName));
            dispatch(setAppBusy(true));
            const request = { id: actionId, inputText: inputContent, inputLanguageId: settings?.languageConfig.defaultInputLanguage || 'auto', outputLanguageId: settings?.languageConfig.defaultOutputLanguage || 'auto' };
    ...
    if (!promptGroups || tabNames.length === 0) {
        return <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--ink-3)' }}>Loading actions…</div>;
    }
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', borderBottom: '1px solid var(--line)', overflowX: 'auto', flexShrink: 0 }}>
                {sortedGroups.map((group) => (
                    <button
                        key={group.groupId}
                        onClick={() => { dispatch(setActiveActionsTab(group.groupId)); logger.logInfo(`Tab changed to ${group.groupId}`); }}
                        disabled={isAppBusy}
                        style={{ padding: 'var(--space-2) var(--space-3)', border: 'none', borderBottom: group.groupId === activeTab ? '2px solid var(--teal)' : '2px solid transparent', background: 'none', cursor: isAppBusy ? 'not-allowed' : 'pointer', color: group.groupId === activeTab ? 'var(--teal)' : 'var(--ink-2)', fontWeight: group.groupId === activeTab ? 700 : 400, whiteSpace: 'nowrap', fontSize: '0.85rem' }}
                    >
                        {group.groupName}
                    </button>
                ))}
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: 'var(--space-2)', display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', alignContent: 'flex-start' }}>
                {activeGroup &&
                    Object.entries(activeGroup.prompts).map(([promptId, prompt]) => (
                        <button
                            ...
                            disabled={isAppBusy}
                            style={{ padding: 'var(--space-1) var(--space-3)', border: '1px solid var(--teal)', borderRadius: 'var(--radius-pill)', background: 'none', color: 'var(--teal)', cursor: isAppBusy ? 'not-allowed' : 'pointer', opacity: isAppBusy ? 0.5 : 1, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.04em', minWidth: 120 }}
                        >
                            {prompt.name}
                        </button>
                    ))}
