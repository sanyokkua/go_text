...
export const processPromptChain = createAsyncThunk<apperr.ChainResultEnv, apperr.ChainRequest, { rejectValue: string; dispatch: AppDispatch }>(
    'run/processPromptChain',
...
export const cancelChain = createAsyncThunk<void, string, { rejectValue: string }>('run/cancelChain', async (runId, { rejectWithValue }) => {
    try {
...
export const runSingleAction = createAsyncThunk<void, RunSingleActionArgs, { rejectValue: string; dispatch: AppDispatch }>(
    'run/runSingleAction',
    async ({ actionId, inputText, settings }, { dispatch, rejectWithValue }) => {
        try {
