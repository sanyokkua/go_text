...
test.describe('Manual theme switching – Appearance tab', () => {
    ...
    test('selecting Light removes the .dark class from <html>', async ({ page }) => {
        ...
        // Start in dark mode so the transition is testable.
        await page.addInitScript(() => {
            globalThis.localStorage.setItem('ui.theme', 'dark');
        });
        await openAppearanceTab(page);
        await page.waitForFunction(() => document.documentElement.classList.contains('dark'));
    ...
    test('selecting Auto with a light OS emulation removes the .dark class', async ({ page }) => {
        ...
        // Start dark so we can verify the class is removed.
        await page.addInitScript(() => {
            globalThis.localStorage.setItem('ui.theme', 'dark');
        });
    ...
    test('Dark mode persists after page reload via localStorage', async ({ page }) => {
        ...
        const storedTheme = await page.evaluate(() => globalThis.localStorage.getItem('ui.theme'));
        expect(storedTheme).toBe('dark');

        // Act – reload without clearing storage
        await page.reload();
        await page.waitForLoadState('networkidle');

        // Assert – theme is restored on reload
